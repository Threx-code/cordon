...
def identify_language(path: str) -> str | None:
    ...
    such files have no extension. Extension second. Content-based identification
    (shebang) needs the file's bytes and is handled where those are available.

    Cached because inventory and unit production both ask, and a repository has
    far fewer distinct extensions than files.
    """
...
def language_from_interpreter(interpreter: str) -> str | None:
    ...
    # the argument, and this form is more common than a direct path.
    if name == "env":
