...
_NPM: Final = frozenset(
    {
        ...
        "debug", "moment", "request", "async", "underscore", "bluebird", "colors",
        "vue", "webpack", "babel-core", "typescript", "eslint", "prettier",
        "jest", "mocha", "chai", "socket.io", "mongoose", "dotenv", "uuid",
        "body-parser", "cors", "jsonwebtoken", "bcrypt", "redis", "next",
        "node-fetch", "cross-env", "rimraf", "glob", "minimist", "yargs",
        "semver", "tslib", "classnames", "prop-types", "redux", "rxjs",
        "graphql", "apollo-client", "styled-components", "tailwindcss",
        "@babel/core", "@types/node", "esbuild", "vite", "rollup", "postcss",
        "nodemon", "concurrently", "husky", "lint-staged", "cheerio", "puppeteer",
    }
)

_PYPI: Final = frozenset(
    {
        "requests", "urllib3", "numpy", "pandas", "setuptools", "six", "boto3",
        "botocore", "python-dateutil", "certifi", "idna", "charset-normalizer",
        "pyyaml", "click", "jinja2", "markupsafe", "flask", "sqlalchemy",
        "pytest", "attrs", "packaging", "typing-extensions", "cryptography",
        "pillow", "scipy", "matplotlib", "scikit-learn", "beautifulsoup4",
        "lxml", "colorama", "tqdm", "protobuf", "grpcio", "google-api-core",
        "pyparsing", "wheel", "pip", "virtualenv", "tomli", "rich", "httpx",
        "pydantic", "fastapi", "uvicorn", "starlette", "celery", "redis",
        "psycopg2", "pymongo", "openpyxl", "selenium", "paramiko", "pyjwt",
        "tensorflow", "torch", "transformers", "openai", "anthropic", "aiohttp",
    }
)

_CARGO: Final = frozenset(
    {
        "serde", "serde_json", "tokio", "rand", "libc", "log", "regex", "clap",
        "reqwest", "anyhow", "thiserror", "futures", "chrono", "itertools",
        "lazy_static", "once_cell", "bytes", "hyper", "syn", "quote",
        "proc-macro2", "tracing", "uuid", "base64", "sha2", "hex", "num-traits",
    }
)
...
_GO: Final = frozenset(
    {
        ...
        "github.com/spf13/cobra", "github.com/spf13/viper", "github.com/gin-gonic/gin",
        "github.com/gorilla/mux", "google.golang.org/grpc", "google.golang.org/protobuf",
        "github.com/pkg/errors", "gopkg.in/yaml.v3", "github.com/google/uuid",
        "github.com/aws/aws-sdk-go", "golang.org/x/crypto", "golang.org/x/net",
    }
)

_RUBYGEMS: Final = frozenset(
    {
        "rails", "rake", "bundler", "rspec", "nokogiri", "puma", "sinatra",
        "activesupport", "activerecord", "devise", "sidekiq", "pg", "mysql2",
        "redis", "json", "rubocop", "faraday", "httparty", "jwt", "dotenv",
    }
)

_COMPOSER: Final = frozenset(
    {
        "symfony/console", "monolog/monolog", "guzzlehttp/guzzle", "phpunit/phpunit",
        "laravel/framework", "doctrine/orm", "psr/log", "nikic/php-parser",
        "vlucas/phpdotenv", "ramsey/uuid", "firebase/php-jwt",
    }
)

_MAVEN: Final = frozenset(
    {
        "org.slf4j:slf4j-api", "com.google.guava:guava", "junit:junit",
        ...
        "ch.qos.logback:logback-classic", "org.mockito:mockito-core",
    }
)

_NUGET: Final = frozenset(
    {
        "newtonsoft.json", "serilog", "automapper", "dapper", "nunit", "xunit",
        "moq", "polly", "fluentvalidation", "mediatr", "restsharp",
    }
)
...
_KNOWN_NEIGHBOURS: Final[dict[str, frozenset[str]]] = {
    "npm": frozenset(
        {
            "preact", "inferno", "reactor", "lodash-es", "lodash.merge",
            "async-mutex", "colorette", "debounce", "express-session",
            "node-forge", "axios-retry", "chalk-template", "commander-js",
            "vue-router", "webpack-cli", "jest-cli", "uuid-js", "cors-anywhere",
        }
    ),
    "pypi": frozenset(
        {
            "requests-oauthlib", "requests-toolbelt", "urllib3-secure-extra",
            "numpy-financial", "pandas-gbq", "pytest-cov", "pytest-django",
            "flask-cors", "flask-login", "click-default-group", "redis-py-cluster",
            "types-requests", "boto", "botocore-stubs", "torch-audio",
        }
    ),
